...
- [ ] Hide/remove the "Create a Board" button + modal in `components/features/MyBoardsSection.tsx` and the Step-1 `+` button in `app/(dashboard)/welcome/WelcomeClient.tsx`
- [ ] Hide/remove the "Join a Board" invite-code input in `MyBoardsSection.tsx` (lines ~357-382) — this is the *in-dashboard* join-another-board feature for already-registered users, which you don't need since nobody should belong to more than one of the two teams. Keep the *registration-time* code entry (`app/(auth)/register`) fully intact — that's the self-serve onboarding path you're using
- [ ] Short-circuit `createBoard()` server action in `app/actions/boards.ts` to reject/no-op regardless of UI state (defense in depth)
- [ ] Tighten the `boards_insert_user` RLS policy (from `supabase/migrations/20260616000004_boards_system.sql`) so only a service-role/admin context can insert a `boards` row — don't rely on the UI/server-action gate alone
